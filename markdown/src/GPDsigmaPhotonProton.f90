!********************************************************************************
!>
!  Modulo que calcula $\sigma_{\gamma P \rightarrow J/\psi P}$ através de GPD's em NNLO
!
!### References
...
! Interface para códigos em fortran 77
!interface
...
contains
    ...
    Subroutine  AmpPhotonProton(Amplitude)  
        ...
        !f => IntImg
        call dgauss8( IntImg, tmin, 0.99_wp, epsrel, AmpIntImg, ier, err)
        
        !f => IntReal
        call dgauss8( IntReal, tmin, 0.99_wp, epsrel, AmpIntReal, ier, err)
    ...
    function fq(y) result(Resultfq)
        complex(8):: y, fq_1, fq_2, fq_3, fq_4, fq_5, fq_6, fq_7, fq_8, fq_9, fq_10, Resultfq
        ...
        fq_6     = (1+2*y) * (((log(-y))**2)/(1+y) - (log(1+y))**2/(y))
        fq_7     = ((3-4*y+16*y*(1+y))/(4*y*(1+y)))
        fq_8     = cdli2(1+2*y)
        fq_9     = ((7+4*y+16*y*(1+y))/(4*y*(1+y)))
        fq_10    = cdli2(-1-2*y)

        Resultfq = fq_1*fq_2-fq_3+fq_4+fq_5+fq_6+fq_7*fq_8-fq_9*fq_10
    end function fq

    function Tg(x) result(ResultTg)
        real(8)              :: x
        complex(8)           :: ResultTg, ResultTgLO, ResultTgNLO

        complex(8):: epsilon
        !print *, 'alhpas = ', alpha_s 
        epsilon = xi*dcmplx(0.d0, 1.d-8)
        ResultTgLO = (xi/((x-xi+epsilon)*(x+xi-epsilon))) * alpha_s
        epsilon = xi*dcmplx(0.d0, 1.d-5)
        ResultTgNLO = (xi/((x-xi+epsilon)*(x+xi-epsilon)))*((alpha_s**2)/(4*pi))*fg((x-xi+epsilon)/(2*xi))
        ResultTg = ResultTgLO+ResultTgNLO
    end function Tg 
    function fg(y) result(Resultfg)

        complex(8)   :: Resultfg
        complex(8):: y, fg_1, fg_2, fg_3, fg_4, fg_5, fg_6, fg_7, fg_8, fg_9, fg_10
        complex(8):: fg_11, fg_12, fg_13, fg_14, fg_15, fg_16, fg_17, fg_18, fg_19
        complex(8):: fg_20, fg_21, fg_22, fg_23, fg_24, fg_25, fg_26, fg_27, fgt_1
        complex(8):: fgt_2, fgt_3, fgt_4, fgt_5
        ...
        fg_7     = 8*c_1
        fg_8     = (2+y*(1+y)*(25+88*y*(1+y)))*c_1/(48*(y**2)*((1+y)**2))
        fg_9     = (10+y*(1+y)*(7-52*y*(1+y)))*c_2/(24*(y**2)*((1+y)**2))
        fg_10    = c_1*(1+6*y*(1+y)*(1+2*y*(1+y)))/(y*(1+y)*((1+2*y)**2))
        fg_11    = c_2*((1+2*y)**2)/(y*(1+y))
        fg_12    = PI * (SQRT(-y*(1+y))/(y*(1+y)))*((7/2.d0)*c_1-3*c_2)
    ...
    function a_1(y) result(Resulta_1)
        complex(8):: y, Resulta_1

        Resulta_1 = 5.0D0 + (16.0D0*y) - (6.0D0 / (1.D0+y))
        Resulta_1 = Resulta_1 + (1.0D0 / (1.0D0+2.0D0*y)**2)
        Resulta_1 = (c_1/4.0D0) * (Resulta_1 - (5.0D0 / (1.0D0+2.0D0*y)))
        Resulta_1 = Resulta_1 - (c_2/2.0D0) * (2.0D0 + (3.0D0/y))
        Resulta_1 = Resulta_1 - (c_2/2.0D0) * ((8.0D0*y) - (1.0D0 / (1.0D0+y)))
    end function a_1 

    function a_2(y) result(Resulta_2)
        complex(8):: y, Resulta_2

        Resulta_2 = 12.0D0 + (9.0D0/y) + (64.0D0*y) - (2.0D0 / (1.0D0+y)**2)
        Resulta_2 = (c_1/8.0D0) * (Resulta_2 + (21.0D0 / (1.0D0+y)) - (4.0D0 / (1.0D0+2.0D0*y)))
        Resulta_2 = Resulta_2 - (c_2/4.0D0) * (8.0D0 + (3.0D0/y**2) + (11.0D0/y) )
        Resulta_2 = Resulta_2 - (c_2/4.0D0) * ((32.0D0*y) - (2.0D0 / (1.0D0+y)**2) + (9.0D0 / (1.0D0+y)))
    end function a_2
